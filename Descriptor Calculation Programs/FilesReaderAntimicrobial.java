...
public class FilesReaderAntimicrobial 
{
	...
	public static PrintWriter inactiveout; // prints uniprot sequences to InactiveSequencesAntimicrobial
	...
	public static void main (String[] args) throws IOException
	{
		...
		inactivebr = new BufferedReader(new FileReader("InactiveSequencesTotalAMP.txt"));
		inactiveout = new PrintWriter(new BufferedWriter(new FileWriter("InactiveSequencesTotalAMP.txt")));
		out = new PrintWriter(new BufferedWriter(new FileWriter("TrainingSequencesAMP.txt")));
		inactiveout2 = new PrintWriter(new BufferedWriter(new FileWriter("InactiveSequencesAMP.txt")));
		...
		String line;
		int[] lengths = new int[176]; // keep track of how many peptides have each length 
        ...
        ArrayList<String>[] inactiveSequences = new ArrayList[lengths.length]; //store inactive sequences by length (length is index)
        ...
        for (int i = 0; i< inactiveSequences.length; i++)
        	System.out.println(i + ": " + inactiveSequences[i].size() + ", " + lengths[i]);
        inactiveout.close();
        
		// then write inactive sequences to TrainingSequences.txt randomly
        
        int totalInactiveData = 0;
        for (int i = 5; i < lengths.length; i++) // for each specific length
        {
